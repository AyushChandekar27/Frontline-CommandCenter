...
const MOVE_INTERVAL = 5000; // was 3000 — 40% fewer ticks
const ALERT_SPEED = 0.0008;
const RETURN_SPEED = 0.0007;
const ASSIGNED_TICKS = 2;
const ON_SITE_TICKS = 4;

export function useTeamSimulation(canReadAlerts = false) {
    const [locations, setLocations] = useState([]);
    const stateRef = useRef([]);
    const timer = useRef(null);
    const initializedRef = useRef(false); // guard against double-init in StrictMode
            ...
            try {
                ...
                const initial = teams
                    ...
                    .map(t => {
                        ...
                        const status    = asgn?.status === 'ON_SITE'  ? 'ON_SITE'
                            : asgn?.status === 'EN_ROUTE' ? 'EN_ROUTE'
                                : hasTarget                   ? 'ASSIGNED'
                                    :                               'AVAILABLE';
                        return {
                            teamId: t.id,
                ...
                // retry once after 5s — don't loop indefinitely
        ...
        stateRef.current = stateRef.current.map(loc => {
            if (loc.status === 'ASSIGNED') {
                ...
                if (assignedTicks >= ASSIGNED_TICKS && loc._targetLat && loc._targetLng) {
                    return { ...loc, status: 'EN_ROUTE', speedKmh: parseFloat((Math.random() * 12 + 48).toFixed(1)), _assignedTicks: assignedTicks, lastUpdateAt: new Date().toISOString() };
                }
                return { ...loc, speedKmh: 0, _assignedTicks: assignedTicks, lastUpdateAt: new Date().toISOString() };
            }
            ...
            if (loc.status === 'RETURNING') {
                ...
                if (dist < 0.001) {
                    return { ...loc, status: 'AVAILABLE', speedKmh: 0, latitude: loc._baseLat, longitude: loc._baseLng, assignedAlertLat: null, assignedAlertLng: null, _targetLat: null, _targetLng: null, _assignedTicks: 0, _onSiteTicks: 0, lastUpdateAt: new Date().toISOString() };
                }
            ...
            if (loc.status === 'ON_SITE') {
                ...
                if (onSiteTicks >= ON_SITE_TICKS) {
                    return { ...loc, status: 'RETURNING', speedKmh: parseFloat((Math.random() * 12 + 35).toFixed(1)), _onSiteTicks: 0, lastUpdateAt: new Date().toISOString() };
                }
            ...
            if (loc.status === 'EN_ROUTE' && loc._targetLat && loc._targetLng) {
                ...
                const dist = Math.sqrt(dLat * dLat + dLng * dLng);
                if (dist < 0.001) {
                    return { ...loc, status: 'ON_SITE', speedKmh: 0, latitude: loc._targetLat, longitude: loc._targetLng, _onSiteTicks: 0, lastUpdateAt: new Date().toISOString() };
                }
                return moveToward(loc, loc._targetLat, loc._targetLng, ALERT_SPEED, 'EN_ROUTE', 50, 70);
            }

            return { ...loc, status: 'AVAILABLE', speedKmh: 0, lastUpdateAt: new Date().toISOString() };
        });
        setLocations(stateRef.current.map(clean));
    }

    function assignTarget(teamId, alertLat, alertLng) {
        stateRef.current = stateRef.current.map(l =>
            l.teamId === teamId
                ? { ...l, _targetLat: alertLat, _targetLng: alertLng, assignedAlertLat: alertLat, assignedAlertLng: alertLng, status: 'ASSIGNED', speedKmh: 0, _assignedTicks: 0, _onSiteTicks: 0, lastUpdateAt: new Date().toISOString() }
                : l
        );
        setLocations(stateRef.current.map(clean));
    }
...
function moveToward(loc, targetLat, targetLng, step, status, minSpeed, maxSpeed) {
    ...
    return {
        ...loc,
        latitude:  loc.latitude  + (dLat / dist) * step,
        longitude: loc.longitude + (dLng / dist) * step,
        status,
